...
- (void) rePositionViews {
    float topSafeArea = 20;
    if (@available(iOS 11.0, *)) {
        topSafeArea = [[[UIApplication sharedApplication] delegate] window].safeAreaInsets.top;
    }
    if ([_browserOptions.toolbarposition isEqualToString:kInAppBrowserToolbarBarPositionTop]) {
        [self.webView setFrame:CGRectMake(self.webView.frame.origin.x, TOOLBAR_HEIGHT + topSafeArea, self.webView.frame.size.width, self.webView.frame.size.height)];
